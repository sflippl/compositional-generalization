...
class LightningTrainer:
    ...
    def fit(self, train_data, val_data):
        ...
        if self.cfg.auto_lr_find:
            tuner = Tuner(trainer)
            tuner.lr_find(self.module, train_loader, num_training=200, max_lr=self.cfg.lr)
...
class Module(L.LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.analyse_on_this_epoch():
            wd = hydra.core.hydra_config.HydraConfig.get().runtime.output_dir
            dfs, names = self.analysis.on_validation_epoch_end()
    ...
    def configure_optimizers(self):
        return optim.SGD(self.model.parameters(), lr=self.lr, momentum=self.cfg.momentum)
